...
class DataBaseHelper {
  ...
  Future<Database> initDb() async {
    ...
    return await openDatabase(
      ...
      onCreate: (Database db, int version) async {
        await db.execute(
            'CREATE TABLE $tableName ($userId INTEGER PRIMARY KEY AUTOINCREMENT, $userName TEXT NOT NULL, $userEmail TEXT NOT NULL, $userPhone TEXT NOT NULL)');
        _database = db;
      },
    );
  }
  ...
  Future<void> update(UserModel user,int id) async {
    var db = await database;
    db.update(
      tableName,
      user.toJson(),
